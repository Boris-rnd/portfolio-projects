...
macro_rules! row_impl {
    ($R: ty $(,$tt: tt)?) => {
        impl<'a $(, $tt)?, C: Shape, T, Rs: Stride, Cs: Stride, ColRange: IntoRange<IdxInc<C>, Len<C>: 'a>>
            MatIndex<Idx<$R>, ColRange> for MatRef<'a, T, $R, C, Rs, Cs>
        {
            ...
            fn get(this: Self, row: Idx<$R>, col: ColRange) -> Self::Target {
                ...
                let ncols =
                    unsafe { ColRange::Len::<C>::new_unbound(col.end.unbound() - col.start.unbound()) };
                this.subcols(col.start, ncols).row(row)
            }
        ...
        impl<'a $(, $tt)?, C: Shape, T, Rs: Stride, Cs: Stride, ColRange: IntoRange<IdxInc<C>, Len<C>: 'a>>
            MatIndex<Idx<$R>, ColRange> for MatMut<'a, T, $R, C, Rs, Cs>
        {
            ...
            fn get(this: Self, row: Idx<$R>, col: ColRange) -> Self::Target {
                ...
                let ncols =
                    unsafe { ColRange::Len::<C>::new_unbound(col.end.unbound() - col.start.unbound()) };
                this.subcols_mut(col.start, ncols).row_mut(row)
            }
...
macro_rules! col_impl {
    ($C: ty $(,$tt: tt)?) => {
        impl<'a $(, $tt)?, R: Shape, T, Rs: Stride, Cs: Stride, RowRange: IntoRange<IdxInc<R>, Len<R>: 'a>>
            MatIndex<RowRange, Idx<$C>> for MatRef<'a, T, R, $C, Rs, Cs>
        {
            ...
            fn get(this: Self, row: RowRange, col: Idx<$C>) -> Self::Target {
                ...
                this.subrows(row.start, nrows).col(col)
            }
        ...
        impl<'a $(, $tt)?, R: Shape, T, Rs: Stride, Cs: Stride, RowRange: IntoRange<IdxInc<R>, Len<R>: 'a>>
            MatIndex<RowRange, Idx<$C>> for MatMut<'a, T, R, $C, Rs, Cs>
        {
            ...
            fn get(this: Self, row: RowRange, col: Idx<$C>) -> Self::Target {
                ...
                let nrows =
                    unsafe { RowRange::Len::<R>::new_unbound(row.end.unbound() - row.start.unbound()) };
                this.subrows_mut(row.start, nrows).col_mut(col)
            }
...
macro_rules! idx_impl {
    (($R: ty $(,$Rtt: tt)?),($C: ty $(,$Ctt: tt)?)) => {
        impl<'a $(, $Rtt)? $(, $Ctt)?, T, Rs: Stride, Cs: Stride>
            MatIndex<Idx<$R>, Idx<$C>> for MatRef<'a, T, $R, $C, Rs, Cs> {
            type Target = &'a T;
            ...
            unsafe fn get_unchecked(this: Self, row: Idx<$R>, col: Idx<$C>) -> Self::Target {
                this.at_unchecked(row, col)
            }
        }

        impl<'a $(, $Rtt)? $(, $Ctt)?, T, Rs: Stride, Cs: Stride>
            MatIndex<Idx<$R>, Idx<$C>> for MatMut<'a, T, $R, $C, Rs, Cs> {
            type Target = &'a mut T;

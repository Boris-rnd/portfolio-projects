	...
	impl<T: ComplexField> pulp::WithSimd for Impl<'_, T> {
		...
		fn with_simd<S: pulp::Simd>(self, simd: S) -> Self::Output {
			...
			for i in M.indices() {
				...
				out[i] = mul_real(&sum, &recip(&from_usize::<T::Real>(non_nan_count_total)));
			}
		}
	}
	...
	impl<T: ComplexField> pulp::WithSimd for Impl<'_, T> {
		...
		fn with_simd<S: pulp::Simd>(self, simd: S) -> Self::Output {
			...
			for i in M.indices() {
				...
				let sum = real(&simd.reduce_sum(sum0.0));
				...
				if non_nan_count == 0 {
					...
					out[i] = mul_real(&sum, &recip(&from_usize::<T::Real>(non_nan_count - 1)));
				}
			}
		}
	}
...
fn col_mean_row_major_propagate_nan<T: ComplexField>(out: ColMut<'_, T>, mat: MatRef<'_, T, usize, usize, isize, ContiguousFwd>) {
	...
	impl<T: ComplexField> pulp::WithSimd for Impl<'_, T> {
		...
		fn with_simd<S: pulp::Simd>(self, simd: S) -> Self::Output {
			...
			let n = recip(&from_usize::<T::Real>(*N));
			for i in M.indices() {
				let row = mat.row(i).transpose();
...
fn col_mean_ignore_nan_fallback<T: ComplexField>(out: ColMut<'_, T>, mat: MatRef<'_, T>) {
	...
	for i in M.indices() {
		out[i] = mul_real(out[i], recip(from_usize::<T::Real>(non_nan_count[i])));
	}
}
...
fn col_varm_ignore_nan_fallback<T: ComplexField>(out: ColMut<'_, T::Real>, mat: MatRef<'_, T>, col_mean: ColRef<'_, T>) {
	...
	for j in N.indices() {
		for i in M.indices() {
			let val = copy(mat[(i, j)]);
			let col_mean = copy(col_mean[i]);
			let nan = is_nan(val);
			let val = if nan { zero::<T::Real>() } else { abs2(val - col_mean) };
	...
	for i in M.indices() {
		...
		if non_nan_count == 0 {
			...
			out[i] = mul_real(out[i], recip(from_usize::<T::Real>(non_nan_count - 1)));
		}
	}
}
...
fn col_varm_propagate_nan_fallback<T: ComplexField>(out: ColMut<'_, T::Real>, mat: MatRef<'_, T>, col_mean: ColRef<'_, T>) {
	...
	for j in N.indices() {
		for i in M.indices() {
			let val = abs2(mat[(i, j)] - col_mean[i]);
			out[i] = out[i] + val;
		}
	}
...
mod tests {
	...
	fn test_meanvar_propagate() {
		...
		assert!(
			row_var
				== row![
					abs2(&(A[(0, 0)] - row_mean[0])) + abs2(&(A[(1, 0)] - row_mean[0])),
					abs2(&(A[(0, 1)] - row_mean[1])) + abs2(&(A[(1, 1)] - row_mean[1])),
				]
		);
		...
		assert!(
			col_var
				== col![
					abs2(&(A[(0, 0)] - col_mean[0])) + abs2(&(A[(0, 1)] - col_mean[0])),
					abs2(&(A[(1, 0)] - col_mean[1])) + abs2(&(A[(1, 1)] - col_mean[1])),
				]
		);
	}

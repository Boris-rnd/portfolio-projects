...
fn iterate_lanczos<T: ComplexField>(
	...
	for j in start..end + 1 {
...

		{
			...
			for i in 0..j - 1 {
				let r = P.col(i).adjoint() * Pnext.rb();
				zip!(Pnext.rb_mut(), P.col(i)).for_each(|unzip!(y, x): Zip!(&mut _, &_)| *y = *y - r * *x);
			}
			{
				...
				loop {
					...
					for i in 0..j - 1 {
						if !converged[i] {
							...
							let r = P.col(i).adjoint() * Pnext.rb();
							zip!(Pnext.rb_mut(), P.col(i)).for_each(|unzip!(y, x): Zip!(&mut _, &T)| *y = *y - r * *x);

							converged[i] = abs(r) < f * Pnext.norm_l2();
						}
					}
					if all_true {
						break;
			...
			if norm > zero() {
				...
				zip!(&mut Pnext).for_each(|unzip!(v)| *v = mul_real(*v, norm_inv));
			} else {
				break;
			}
			H[(j - 1, j - 1)] = from_real(norm);
		}
...

		{
			...
			for i in 0..j {
				let r = Q.col(i).adjoint() * Qnext.rb();
				zip!(Qnext.rb_mut(), Q.col(i)).for_each(|unzip!(y, x): Zip!(&mut _, &T)| *y = *y - r * *x);
			}
			{
				...
				loop {
					...
					for i in 0..j {
						if !converged[i] {
							...
							converged[i] = abs(r) < f * Qnext.norm_l2();
						}
					}
					if all_true {
						break;
			...
			if norm > zero() {
				let norm_inv = recip(norm);
				zip!(&mut Qnext).for_each(|unzip!(v)| *v = mul_real(*v, norm_inv));
			} else {
				break;
			}
			H[(j - 1, j)] = from_real(norm);
		}
	}
}
...
pub fn partial_svd_imp<T: ComplexField>(
	...
	if max_dim < Ord::min(m, n) {
		...
		if f > min_positive() {
			let f = recip(f);
			zip!(Q.rb_mut().col_mut(0), v0).for_each(|unzip!(y, x): Zip!(&mut _, &_)| *y = mul_real(*x, f));
		} else {
			let n0 = n as u32;
			let n1 = (n >> 32) as u32;
			...
			zip!(Q.rb_mut().col_mut(0)).for_each(|unzip!(y)| *y = copy(f));
		}
		...
		for _ in 0..restarts {
			...
			let Hmm = copy(H[(max_dim - 1, max_dim)]);
			...
			for i in nev..max_dim {
				let group;
				if k < ideal_size && abs(residual[i]) > tol {
					group = Group::Retain;
					k += 1;
				} else {
					group = Group::Purge;

...
use crate::{
    ...
    get_state,
    pipelines::{
        fonts::{FontRenderer, CHAR_HEIGHT, CHAR_WIDTH},
        rect_texture::img_load_from_file,
    },
            ...
            WindowEvent::KeyboardInput {
                ...
                match event.physical_key {
                    winit::keyboard::PhysicalKey::Code(c) => match c {
                        KeyCode::KeyZ => {
                            let t = "hello world";
                            get_state().pipelines.fonts.add_text(
                                vec2f(100., CHAR_HEIGHT as _),
                                t,
                                48.,
                            );
                            get_state().pipelines.rect_pipeline.push_rect(RawRect::new(
                                100.,
                                000.,
                                t.len() as f32 * CHAR_WIDTH as f32,
                                CHAR_HEIGHT as f32,
                            ))
                        }
                        KeyCode::Digit0 => {

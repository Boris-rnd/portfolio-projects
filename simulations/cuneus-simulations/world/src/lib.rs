...
impl GameWorld {
    ...
    fn block_iter_inner(&mut self, pos: IVec3, map_data: Option<MapData>) -> Option<(MapData, VoxelChunkID)> {
            ...
            || pos.z < self.root_size() as i32
        {
            ...
            for depth in 1..100 {
                ...
                log::trace!(
                    "{depth}: {local_pos:?} -> Chunk {map_data_id:?} (offset: {parent_pos:?}) with size: {}",
                    chunk_size
                );
                ...
                match self.get_data_in_chunk(map_data_id, local_pos_idx) {
                    ...
                    None => {
                        ...
                        match map_data {
                            ...
                            None => {
                                log::trace!(
                                    "No data found in chunk {map_data_id:?} at local pos {local_pos:?}, returning None"
                                );
                                log::trace!(
                                    "{local_pos}={} in {:b}",
                                    local_pos_idx.idx,
    ...
    pub fn set_map_data(&mut self, map_data_id: MapDataID, value: MapData) {
        ...
        assert!(
            (map_data_id.array_idx as usize)
                < self.block_data[map_data_id.array_array_idx as usize].len(),
            "{map_data_id:?} {value:?}"
        );
    ...
    pub fn push_map_data(&mut self, map_data_id: MapDataID, value: MapData) {
        self.block_data[map_data_id.array_array_idx as usize].push(value.pack()); // _within_capacity
    }
    #[track_caller]
    ...
    fn alloc_new_chunk(
        ...
        log::trace!(
            "Chunk {parent_chunk_id:?} at {idx_in_parent:?}={} is empty, filling with new chunk {} at offset {} in block data array",
            idx_in_parent.idx,
            self.voxel_chunks.len(),
            self.block_data[0].len()
        );

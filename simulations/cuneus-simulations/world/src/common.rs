...
impl LocalPos {
    ...
    pub fn new(x: u32, y: u32, z: u32) -> Self {
        ...
        Self {
            idx: (x & CHUNK_MASK32)
                | ((y & CHUNK_MASK32) << CHUNK_MASK_SIZE)
                | ((z & CHUNK_MASK32) << (CHUNK_MASK_SIZE * 2)),
        }
    }
    pub fn x(&self) -> u32 {
...
impl ToLocalPos for IVec3 {
    ...
    fn to_local_pos(&self) -> Result<LocalPos> {
        if !(self.x >= 0
            && self.x < CHUNK_SIZE32 as i32
            && self.y >= 0
            && self.y < CHUNK_SIZE32 as i32
            && self.z >= 0
            && self.z < CHUNK_SIZE32 as i32)
        {
            return Err(anyhow!(
                "Local position out of bounds: {}, {}, {}",
                self.x,
...
impl ToLocalPos for UVec3 {
    ...
    fn to_local_pos(&self) -> Result<LocalPos> {
        assert!(
            self.x < CHUNK_SIZE32 && self.y < CHUNK_SIZE32 && self.z < CHUNK_SIZE32,
            "Local position out of bounds: {}, {}, {}",
            self.x,
            self.y,
            self.z

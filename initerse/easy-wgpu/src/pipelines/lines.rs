...
impl LinesPipeline {
    pub fn new() -> Self {
        ...
        let inst_buf = State::buffer(&state.device, &instances, BufferUsages::MAP_WRITE | BufferUsages::VERTEX | BufferUsages::COPY_SRC);
    ...
    pub fn add_line(&mut self, line: Line) -> u32 {
        ...
        *n += 2;
        ...
        let state = get_wgpu_state();
        State::write_to_buffer_with_staging(&state.device, &state.queue,buf, (idx*8 as u32) as _, &[line]);
        *n
    }
    pub fn append_lines(&mut self, lines: &[Line]) -> u32 {
        ...
        *n += 2*lines.len() as u32;
        ...
        let state = get_wgpu_state();
        State::write_to_buffer_with_staging(&state.device, &state.queue,buf, (idx*8 as u32) as _, lines);
        *n
    }
}

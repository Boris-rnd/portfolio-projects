...
impl PipelineManager {
    pub fn new(device: &Device, config: &SurfaceConfiguration, queue: &Queue) -> Self {
        ...
        let mut rect_texture_pipeline = RectangleTexturePipeline::new(
            device,
            config,
            queue,
        ...
        let raw_compute_pipeline =
            compute::ComputePipeline::create_compute_pipeline(device, "shaders/compute.wgsl", None);
...
macro_rules! create_texture {
    ($width: expr,$height:expr, $usage: expr) => {{
        let state = $crate::get_wgpu_state();
...
macro_rules! command {
    ($closure: expr) => {
        ...
        let mut closure: Box<dyn FnMut(&mut wgpu::CommandEncoder)> = Box::new($closure);
        closure(&mut encoder);
